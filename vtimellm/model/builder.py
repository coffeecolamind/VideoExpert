...
def load_VExpert_lora(model, lora_path):
    non_lora_trainables_path = os.path.join(lora_path, 'non_lora_trainables.bin')
    if os.path.exists(non_lora_trainables_path):
        non_lora_trainables = torch.load(non_lora_trainables_path, map_location='cpu')
        non_lora_trainables = {(k[11:] if k.startswith('base_model.') else k): v for k, v in non_lora_trainables.items()}
    ...
    print('Loading VExpert_LoRA weights...')

    VExpert_lora_trainables_path = os.path.join(lora_path, 'pytorch_model.bin')
    if os.path.exists(VExpert_lora_trainables_path):
        VExpert_lora_trainables = torch.load(VExpert_lora_trainables_path, map_location='cpu')
        model.load_state_dict(VExpert_lora_trainables, strict=False)
...
def load_pretrained_model(args, stage2=None, stage3=None):
    ...
    loc_token_idx = tokenizer("[LOC]", add_special_tokens=False).input_ids[0]

    kwargs['loc_token_idx'] = loc_token_idx
    ...
    model.get_model().initialize_Loc_Head_modules(model_args=args)

    if stage2 is not None:
        print('Loading stage2 weights...')
        # model = load_lora(model, stage2)
        model = load_VExpert_lora(model, stage2)
