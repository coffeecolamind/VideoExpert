...
class HybridDataset(Dataset):
    def __init__(self, data_path: str,
        ...
        self.samples_per_epoch = data_args.samples_per_epoch  ## 这里是按照 step 来做 50000 步
        self.tasks = data_args.tasks.split("||")  # TODO: change to tasks?
        task_sample_rate = data_args.task_sample_rate  ## 这里是多个 dataset 的采样比例
        s = sum(task_sample_rate)
        self.task_sample_rate = [float(x)/s for x in task_sample_rate]  ## 这里归一化到 %比率
        assert len(self.task_sample_rate) == len(self.tasks)  ## 这里确定了任务，以及每个任务采样的比例
        ...
        for task in self.tasks:  ## 有几个 task，分别处理
            task_data = getattr(data_args, task + '_data', '')
            datasets = []
            sample_counts = []
            for data in task_data.split('||'):  ## 一个 task 中，还会包含多个不同的数据集
                dataset = ds_dict[task][data](data_path, tokenizer, data_args)  ## 在这里进入到 各个 dataset 里面去进行设置
